\documentclass[calculation_methods, och]{SCWorks1}
...
% Тема работы
\title{Отчёт по практической подготовке}
...
\satitle{профессор, д.ф.-м.н., доцент} %должность, степень, звание
\saname{М.\,Ю.\,Игнатьев}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=12cm]{График времени выполнения от n.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=9cm]{For_newton.png}
...
int main() {
    ...
    int iter_used = 0;
    int success = qr_algorithm(A, eigenvalues, eigenvectors, n, 
                    eps, max_iter, &iter_used);
